...
class TheNode(object):
  '''Class to communicate with the balboa robot. Carrick Detweiler 2020'''
    ...
    # This node receives balboaLL messages from the serial port and publishes them and
    # receives messages from motorSpeeds and publishes them to the serial port
  ...
  def __init__(self):
    ...
    self.ls_publisher = rospy.Publisher('lineSensor', lineSensor, queue_size=10)
    self.ir_publisher = rospy.Publisher('irRange', Float64, queue_size=1)
  ...
  def main_loop(self):
    ...
    while not rospy.is_shutdown():
      ...
      ls_msg = lineSensor()
      ls_msg.sensor0 = self.readFourByteAndChecksum()
      ls_msg.sensor1 = self.readFourByteAndChecksum()
      ls_msg.sensor2 = self.readFourByteAndChecksum()
      ls_msg.sensor3 = self.readFourByteAndChecksum()
      ls_msg.sensor4 = self.readFourByteAndChecksum()
      # Emitter
      self.readFourByteAndChecksum()
      # IR Range Finder
      ir = self.readFourByteAndChecksum()
      ir = ir  * (5.0 /1023.0)
      print("ir ", ir)
      #ir = 170.5 * math.exp(-0.937 * ir)
      ir = 155.79 * math.exp(-0.895 * ir)
      print("dist ", ir)
      ...
      self.ls_publisher.publish(ls_msg)
      self.ir_publisher.publish(ir)
      r.sleep()
